...
class Blur(jt.Module):
    ...
    def execute(self, input):
        return jt.nn.conv2d(input, self._weight, padding=1, groups=input.shape[1])
        ...
        if downsample:
            if fused:
                self.conv2 = jt.nn.Sequential(
                    jt.nn.Conv2d(out_channel, out_channel, kernel_size, padding=padding),
                    jt.nn.Pool(2),
                    jt.nn.LeakyReLU(0.2),
                )
            else:
            ...
            if upsample:
                if fused:
                    self.conv1 = jt.nn.Sequential(
                        ...
                        jt.nn.Conv2d(in_channel, out_channel, kernel_size, padding=padding)
                    )
                else:
                    self.conv1 = jt.nn.Sequential(
                        jt.nn.Upsample(scale_factor=2, mode='nearest'),
    ...
    def __init__(self, fused=True, from_rgb_activate=False):
        ...
        def make_from_rgb(out_channel):
            if from_rgb_activate:
                return jt.nn.Sequential(jt.nn.Conv2d(3, out_channel, 1), jt.nn.LeakyReLU(0.2))

            else:
                return jt.nn.Conv2d(3, out_channel, 1)
